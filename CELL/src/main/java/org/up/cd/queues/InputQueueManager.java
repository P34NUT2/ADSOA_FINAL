...
/**
 * Singleton. Holds incoming messages (responses from server cells) for DispatchThread.
 */
public class InputQueueManager {
    ...
    public void enqueue(BinaryProtocol msg) {
        queue.add(msg);
        logger.debug("[InQueue] Enqueued serviceId={} from={}", msg.getServiceNumber(), msg.getOriginEntity());
    }
